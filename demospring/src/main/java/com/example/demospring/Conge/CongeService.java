...
public class CongeService {
		...
		private final EmployeRepository rep;

		@Autowired
		public CongeService(CongeRepository repcong, DemandeRepository repdem2,EmailService email, EmployeRepository rep) {
			  ...
			  Optional<Demande> demande = repdem.findById(de);
		        ...
		        if ("accepted".equals(choix)) {
		            ...
		            if (demande.get().getMotif() == null || demande.get().getDate_debut() == null || 
						...
						conge = new Conge(demande.get().getMotif(), demande.get().getDate_debut(),
								demande.get().getDate_fin(), demande.get().getEmp());
